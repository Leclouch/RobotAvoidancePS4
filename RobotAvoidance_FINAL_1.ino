...
// Motor
#define BACKUP_TIME 1000       // Time to backup in milliseconds
#define TURN_TIME 500         // Time to turn in milliseconds
...
void setup() {
  ...
  // PS baru (ori)
  PS4.begin("3c:8a:1f:62:01:7a");
  // PS4.begin("3c:8a:1f:62:01:71");
...
void moveWithSpeed(int leftSpeed, int rightSpeed) {
  // Left motor
  if (leftSpeed > 0) {
    ledcWrite(IN1, 0 );   // IN1 - forward
    ledcWrite(IN2, abs(leftSpeed));           // IN2 - backward
  } else {
    ledcWrite(IN1, abs(leftSpeed));
